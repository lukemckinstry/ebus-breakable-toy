...
def write_agency_to_db( obj ):
	...
	if Agency.objects.filter(agency_name=agency_name,
							 agency_id=agency_id).exists():
		a = Agency.objects.get(agency_name=agency_name,
							 agency_id=agency_id)
		a_obj = a
...
def read_write_gtfs( dd, ziplabel ):
	...
	for agencydd in dd['agency']:
		...
		print( agency_id, agency_uuid )
		
		if 'agency_id' in next(iter(dd['routes']), None):
			#print( 'agency_id is in routes' )
			uu_routes = list(set([t['route_id'] for t in dd['routes'] if t['agency_id'] == agency_id]))
		else:
			#print( 'agency_id not in routes' )
			uu_routes = list(set([t['route_id'] for t in dd['routes']]))

		print( 'ready to write # route: ', len(uu_routes) )		
		...
		for route in uu_routes:
			...
			this_route = next(iter([r for r in dd['routes'] if r['route_id'] == route]), None)
			for k in this_route:
				obj[k] = this_route[k] 
			...
			for s in shape_for_route:
				ordered_shape = [{'lon': p['shape_pt_lon'],
								  'lat': p['shape_pt_lat'],
								  'seq': int(p['shape_pt_sequence']),
								  'dst': p.get('shape_dist_traveled',None)
								  } 
									for p in dd['shapes'] if p['shape_id'] == s]
				...
				#shape geo
				shape_geo = [ tuple([float(s['lon']),float(s['lat'])]) for s in sorted_shape ]			
				geos_ls = LineString(shape_geo, srid=4326)
				route_lss.append( geos_ls )

			try:
				obj['route_distance'] = sum(dst_for_route_shapes)/len(dst_for_route_shapes)
			except: 
			...
			####
			cal_object = {i:0 for i in ['monday','tuesday','wednesday','thursday','friday','saturday','sunday']}
			...
			print( 'bad service_ids per route ', len(bad_service_ids_route))
	...
	print( 'the agency has ', len(uu_service_ids), ' unique service ids  ' )

	uu_trips = list(set([t['trip_id'] for t in dd['trips']]))
	print( 'the agency has ', len(uu_trips), ' unique trips ' )

	uu_shapes = list(set([t['shape_id'] for t in dd['shapes']]))
	print( 'the agency has ', len(uu_shapes), ' unique shapes ' )
